...
export async function getQueryHx(userId) {
  const [rows] = await pool.query(
    "SELECT category, inquiry_title, inquiry_date FROM faq WHERE user_id=? ORDER BY faq_id DESC", [userId]
  );
  return rows;
}
...
//faq 작성
export async function addQuery(inquiry_title, user_id, requester, company, email, category, content) {
  await pool.query(
    "INSERT INTO faq (inquiry_title, user_id, requester,company,email, category, content) VALUES (?, ?, ?, ?,?,?,?)",
    [inquiry_title, user_id, requester, company, email, category, content]
  );
}
